...
## 事故四十一：「決定處理哪一季」的退回鏈，不可以用來決定「去外部系統拿哪一季的資料」

**這一則的結論比事故本身重要，先寫在前面：**

> **「決定處理哪一季」的退回鏈，不可以用來決定「去外部系統拿哪一季的資料」。
> 前者退回是方便，後者退回是拿錯資料。**

前者退不到最多是顯示了使用者不想要的一季，一眼看得出；後者退不到就是
**拿到別一季的真實人名，而且看起來完全正常**——不是「缺失被當成正常」，
是「錯的資料被當成正常」，嚴重一級。
...
沙盒季度由 `2028T4` 改成一個更遠的季度之後，自測機 S19 報：

```
對話框：本季職事表未有資料，已建立 0 個主日，事奉欄位全部留空。
實際：13 個主日、其中 0 個 NOT_FOUND、事奉格有值的 26 格
```

三處不符：對話框講建立了 0 個而實際有 13 個；`ROSTER_STATUS` 全部顯示
`OK` 而應該是 `NOT_FOUND`；26 格「事奉欄位」有值，而職事表根本沒有這一季。

最大嫌疑是讀職事表時退回了 `ROSTER_TEST_DATE`（`2027-11-07` → `2027T4`），
把 2027 年的事奉名單填進了那一季。

**查證（結論：那個懷疑是錯的）**

造一個**只有 `2027T4`**（連特別主日標題與一個人名）的假職事表，然後真的
呼叫 `createBlankBulletinWeeks_('2030T2')`，逐格看寫了什麼：
...
**一格都沒有跨季。** 那 26 格是 13 行 × 兩欄：

| 欄 | 為什麼職事表沒有這一季都一定有值 |
|---|---|
| `WEEK_OF_MONTH` | 由**日期本身**算（`Math.floor((日-1)/7)+1`），完全不讀職事表 |
| `PROGRAM_TEMPLATE_ID` | 找不到特別主日時退回 Config `TEMPLATE_DEFAULT` |

**根本原因（三個，全部是「看起來像 bug」的來源，不是跨季）**

1. **`rosterDerivedWeekFieldKeys_()` 被當成兩種意思用。** 它本來的意思是
   「補抓會去填的欄位」，而 S19 拿它去問「有沒有讀過職事表」——那三欄
   之中有兩欄不讀職事表都一定有值，所以答案必然是「有」。
   **一份清單答兩條不同的問題，總有一條會答錯。**

2. **`countRosterStatuses_()` 把空白當成 `OK`：**

   ```js
   var value = String(row.ROSTER_STATUS || '').trim() || ROSTER_STATUS.OK;   // ← 錯
   ```

   空白的意思是「從來沒有算過」，不是「算過而且沒事」。加上
   `selfTestSeedSandboxWeeks_()` 漏寫 `ROSTER_STATUS`，於是一批從未查證過
   的資料列在報告上顯示成「OK 13」；補抓把它們更正成 `NOT_FOUND 13` 之後，
   又看起來像補抓弄壞了東西。**兩個結論都是錯的，而錯的源頭是同一行。**

3. **對話框的「已建立 N 個主日」只講 `added`，不講 `skipped`。** 全部主日
   本來就存在時 `added` 是 0，於是印出「已建立 0 個主日」——工作表上明明
   有 13 行。**「0」與「本來就有」是兩件事，不可以用同一句話講。**

**真正到得到的跨季路徑（查證時另外找到的）**

雖然那一次不是跨季，但跨季**確實有一條路走得通**：

```js
QUARTER_ID: snapshot.quarterId || quarterId,     // ← 舊版
```

職事表沒有 `2030T2` → 走曆法推算 → 逐個日期叫 `readRosterSnapshot_()` →
如果職事表把 `2030-04-07` 登記在 `2027T4` 之下（資料不一致，我們控制不到），
`snapshot.quarterId` 就是 `2027T4` → **一行 2030 年的週報被寫進 2027T4**，
而畫面上完全看不出來。已改成一律用呼叫方指定的季度。

**另一個同源的洞：R-036 之後匯入永遠是 0 行**

`previewContentImport_()` 的主日範圍一直是 `listQuarterServiceDates_()`
（**只讀職事表**）。R-036 之後「職事表未有該季一樣建立得到週報」，但那些
季度匯入時的主日範圍是空的，於是結果永遠是「新增 0、修改 0、刪除 0、
不變 0」——**不是報錯，是靜靜地什麼都不做**，比報錯難發現得多。

修法是加一支 `resolveQuarterServiceDatesWithFallback_()`：職事表 →
**同一季的** `BulletinWeeks` → 曆法推算，並且回報用了哪一個來源。
⚠️ 退回的只是「用哪一份清單」，三個來源全部只看同一個季度，一個都不會跨季。

**修法總結**

1. `rosterOnlyWeekFieldKeys_()`：只可能來自職事表的欄位（目前只有
   `SPECIAL_TYPE`）。要證明「有沒有讀過職事表」只可以用這一份。
2. `countRosterStatuses_()` 加 `UNKNOWN`，空白一律算「未算過」，而且
   `describeRosterStatusCounts_()` 有就一定印出來、
   `listQuartersWithRosterGaps_()` 與頂部橫幅一律當成缺口。
3. `selfTestSeedSandboxWeeks_()` 補寫 `ROSTER_STATUS`。
4. `buildCreateWeeksMessage_()` 同時講「新增幾多」與「略過幾多」；
   狀態分佈改為數「整季在表上的行」，不是「這一次新增的那幾行」。
5. `QUARTER_ID` 一律用呼叫方指定那一季。
6. 新增不變量 **I11**：每一個主日的事奉資料，其來源季度必須等於該主日
   所屬的季度。逐行驗三件事，全部用與被驗邏輯無關的來源對答案。
7. 新增靜態檢查 **`tools/lint-roster-quarter.js`**（兩條規則）：讀職事表的
   入口不准直接、也不准間接收 `resolveWorkingQuarter_()` 的結果。

---

## 事故四十二：靜靜地回一個「全部 0」，比報錯難發現得多

**這一則其實是三件事，但根子是同一個：**

> **上游拿不到資料的時候，把「空的範圍」當成「範圍裏面沒有東西」。**
>
> 「找不到主日清單」與「這一季沒有主日」，在程式碼裏都是一個空陣列；
> 在報告上卻是兩件完全不同的事。第一件要叫人去補，第二件不用做什麼。

---

**症狀一：匯入永遠是四個 0**

R-036 之後，職事表未有該季一樣建立得到週報。但「從內容表匯入」的主日範圍
一直是 `listQuarterServiceDates_()`（**只讀職事表**），於是那些季度：

```
匯入計畫：新增 0、修改 0、刪除 0、不變 0
```

看起來像「已經一致，不用做什麼」，其實是「範圍是空的，什麼都沒有比較過」。
**沒有任何錯誤訊息。** 自測機 S05／S07 就是這樣掛的，而正式環境一樣會發生
——幹事提早建立 `2029T1`，填好內容之後按匯入，得到四個 0。

**症狀二：同一個洞有九個呼叫點**

查下去發現 `listQuarterServiceDates_()` 全專案有九個呼叫點，全部同一個
行為：產生全季週報一份都不產生、團契一個都不產生、季度填寫表建立不到、
演練跑不動、待填清單是空的。**全部都不報錯。**

**症狀三：I03 連續 25 個情境報同一項，而且從來沒有講是哪一項**

`I03` 的 `N01`（本季主日數）的「畫面那個數字」那一路寫死
`listRosterServiceDatesForQuarter_()`，即假設**畫面那個數字一定來自職事表**。
R-036 之後不再成立。於是沙盒季度永遠 `reported=0`、`recount=13`。

而自測機把不變量失敗寫進紀錄那一段**只抄 `expected` 與 `actual`，把
`evidence` 丟掉**——證據早就算好了，在最後一步被扔進垃圾桶。

**根本原因**

1. **「這一季有哪幾個主日」沒有單一入口。** 九個地方各自去問同一條問題，
   於是「答案的定義改了」要改九次，而九次之中改漏八次。
2. **空陣列同時代表兩件事。** 「取不到」與「真的沒有」共用同一個回傳值，
   呼叫方就不可能分得出。
3. **證據算了但沒有印。** 這是最冤枉的一種：`I03` 一直知道是哪一項、
   兩個數字各是幾多，只是那句話沒有傳到看報告的人手上。

**修法**

1. 新增 `src/ServiceDates.gs`：`resolveQuarterServiceDateEntries_()` 是唯一
   入口，四個來源 `ROSTER` → `BULLETIN_WEEKS`（**同一季**）→ `CALENDAR` →
   `NONE`，而且回傳值**講得出自己用了哪一個來源**。
   ⚠️ 退回的只是「用哪一份清單」，四個來源全部只看同一個季度（事故四十一）。
2. `NONE` 是一個**明確的失敗**，呼叫方一定要報「取不到主日清單」，
   不可以靜靜回 0 筆。
3. 九個呼叫點全部改用它，並在對話框／報告講明來源。
4. 靜態防線 `tools/lint-service-dates.js`：除了定義處與那個唯一的包裝，
   `src/` 內不准再直接呼叫 `listQuarterServiceDates_()`。
5. `I03`：`actual` 講得出**是哪一項**，`evidence` 連兩個數字、相差、
   重新數的條件、兩路的獨立程度一齊印；自測機不再丟掉 `evidence`；
   兩路會落到**同一個來源**時報「不適用」而不是假裝驗過。
6. 新增選單「診斷 I03（唯讀）」，逐項印齊，並列明三種下一步。
...
> **`Config` 一格被改，就足以令正式輸出指向錯的地方，而且外表完全看不出
> 分別。凡是會影響外部可見結果的設定，改動必須逐格記錄，並且要有一條
> 不變量拿實際結果反查設定。**
...
`Config` 的 `PUBLISHED_PDF_FILE_ID`（教會網站那條固定連結指向的 PDF）
現值變成了**沙盒自測用**那個檔案的 ID，與 `SELFTEST_MASTER_PDF_FILE_ID`
一模一樣。`AuditLog` 找不到那一格的改動紀錄。

`PublishLog` 佐證沒有造成損害：正式那一次發佈（2027-11-07）記住的
`MASTER_FILE_ID` 仍然是正式那一個；兩行沙盒紀錄（`IS_SELFTEST=TRUE`）
才是指向沙盒檔案。之前加的開跑守門
（`assertSelfTestSandbox_()`：兩個 ID 相同就不准開跑）在寫任何東西之前
攔住了，所以正式檔案內容從來沒有被覆寫。

⚠️ **但如果那道守門沒有裝，教會網站那條連結會被沙盒 PDF 洗掉，而且不會有
任何錯誤訊息**：發佈成功、`PublishLog` 綠色、版本 +1。

**根本原因（查到了，不是猜的）**

`selfTestRunPublish_()`——自測機 S13–S15 與**亂行機的 PUBLISH 動作**都經
它——舊做法是：

```js
var original = getConfig(CONFIG_KEYS.PUBLISHED_PDF_FILE_ID, '');
setConfig(CONFIG_KEYS.PUBLISHED_PDF_FILE_ID, config.masterFileId);   // 寫入沙盒 ID
try {
  return runPublishFlow_(payload);
} finally {
  setConfig(CONFIG_KEYS.PUBLISHED_PDF_FILE_ID, original);            // 還原
}
```

`runPublishFlow_()` 是真實入口，它讀 Config，所以要驗它就要暫時換走那一格
——動機合理，做法有一個 `finally` **救不到**的洞：

**Apps Script 執行被硬中斷時，`finally` 根本不會執行。** 六分鐘上限、
使用者撳停、配額用盡，三種都是硬中斷。自測機的時間預算是 240 秒、亂行機
一跑 20 步，而 S13–S15 與 PUBLISH 全部都要做 Drive IO（慢）——中斷剛好落在
「寫入之後、還原之前」那個窗口，沙盒 ID 就永遠留在正式那一格。

而且**會自我延續**：下一次跑的時候
`original = getConfig(PUBLISHED_PDF_FILE_ID)` 讀到的已經是被污染的值，
「還原」等於把沙盒 ID 再寫一次。

`AuditLog` 為什麼沒有那一筆，有兩個都成立的解釋，**沒有辦法憑現有資料
分辨是哪一個**（結論寫在 docs/待確認事項.md X-1）：

1. 舊版 `setConfig()` 是**先 `setValue()` 再 `appendAuditLog_()`**。中斷落在
   兩者之間，值改了而稽核沒有。
2. 在試算表介面直接打字改一格**不經過 `setConfig()`**，本來就不會有稽核。

**兩個都是真的缺口，所以兩個都修。**

**要點：暫時改一個全域設定去做測試，是一種借東西不還的賭博。**
借的時候寫得再小心（`try/finally`、註解、還原），都擋不住「執行被殺」這一種
——因為那一刻**你的程式碼沒有機會執行任何東西**。正確做法是根本不借：把
「用哪一個檔案」變成一個**只活在這一次執行之內**的參數，執行一死它就跟住死。

**修法**

1. **成因**：`publishConfig_()` 改為經 `effectivePublishMasterFileId_()`，
   優先讀一個記憶體變數 `PUBLISH_MASTER_FILE_ID_OVERRIDE_`。
   `selfTestRunPublish_()` 設那個變數，**Config 由頭到尾一格未動**。
   全 `src/` 現在只剩「建立 master 發佈檔案」一個地方會寫這一格。
2. **稽核**：`setConfig()` 改為**先寫稽核再寫值**（最壞情況是多一筆「打算改
   但沒有改成」的紀錄，那是看得見的），並加一個 `source` 參數標明是誰寫的。
3. **人手改動**：既有的 installable `onEdit` 觸發器加一條分支，把 `Config`
   的人手編輯逐格記成 `CONFIG_MANUAL_EDIT`。只記帳，不阻止。
4. **初始化的邊界**：「自動更新系統種下的預設值」收窄成白名單
   （`CONFIG_UPGRADABLE_DEFAULTS_`，目前只有 `SELFTEST_QUARTER_ID` 一個），
   白名單以外一格都不碰，而且每次更新都要在完成對話框逐條列出。
5. **兩條不變量**：
   - `I12`　`PUBLISHED_PDF_FILE_ID` ≠ `SELFTEST_MASTER_PDF_FILE_ID`
   - `I13`　`PUBLISHED_PDF_FILE_ID` ＝ `PublishLog` **最新一行非自測紀錄**的
     `MASTER_FILE_ID`（從未正式發佈過 → 不適用）
   加上「完成度自我檢測」對應的兩項，兩者相同或對不上一律報 🔴。

⚠️ `I13` 那一條最值得記住：**系統本來就有足夠資料自己發現這件事**——
`PublishLog` 一直記住每一次真實發佈用的是哪一個檔案——**只是從來沒有人
拿它去反查設定**。查不到的東西要加工具；查得到卻沒有查的，要加規則。

---

## 事故四十四：權限系統把唯一有能力修好它的人擋在門外

**結論先寫在前面：**

> **一個權限系統不可以把唯一有能力修好它的人擋在門外。**
>
> 「管理者」與「名單上的人」是兩個不同的集合，而前者一定要包含在後者裏面
> ——不是靠人記得去加，是靠程式**無論如何都放行**。

---

**症狀**

`Config` 的 `WEBAPP_ALLOWED_EMAILS` 只有兩個同工的電郵，**部署者本人不在
裏面**。於是部署者自己開不到自己部署的 Web App，畫面只有一句：

```
沒有使用權限
您目前沒有權限使用本填寫介面。
請聯絡管理者，在 Config 的 WEBAPP_ALLOWED_EMAILS 加入您的電郵。
```

⚠️ 而那個「管理者」就是他自己。唯一改得到那個名單的地方是試算表的
`Config`——幸好那一邊還開得到，否則就是把自己完全鎖死在系統外面。

**根本原因**

```js
if (allowed.length > 0) {
  return allowed.indexOf(caller) !== -1;      // ← 名單非空就只看名單
}
var effective = ...;
return Boolean(effective) && caller === effective;
```

部署者那條路**只在名單空白時才走得到**。設計時的想法是「名單空白＝未設定，
所以只放行部署者」，那個想法本身沒有錯；錯在把它寫成**互斥**的兩條路，於是
一填名單，部署者那條路就消失。

授權範圍本來就應該是「名單 **∪** 部署者」，不是「名單 **或者** 部署者」。

**另一個同樣要緊的問題：訊息幫不到手**

那一頁只講「你沒有權限」，沒有講**你是誰**。而這一類問題最常見的原因根本
不是權限，是**瀏覽器登入了另一個 Google 帳戶**（個人 Gmail 而不是教會帳戶）。
多印一行「你目前登入的是：xxx」，多數情況一眼就看得出。

**修法**

1. 部署者那一條**排在名單判斷之前**，一律放行。
   ⚠️ 排在之後的話，名單非空那條路仍然會先 `return`，等於沒有加——
   這種「加了但永遠走不到」的修法，測試不驗**兩種名單狀態**就抓不到。
2. 「沒有使用權限」那一頁印出目前登入的電郵，並提示可以切換帳戶。
3. 完成度自我檢測加一項：名單不含部署者報 **🟡**（不是 🔴）並講明
   「部署者仍然可以使用，但清單建議加入，以免將來換人時混亂」。
   ⚠️ 報 🟡 的理由：現況是可以用的，不是壞了。報紅等於叫人去修一件沒有
   壞的事，而那會令人日後不再認真看紅燈。

---

## 事故四十五：兩個選單名稱分不出，而分不出的偏偏是「會不會覆寫我填的嘢」

**結論先寫在前面：**

> **功能名稱要答到使用者最怕那條問題。**
>
> 幹事最怕的不是「這一支做什麼」，是「**它會不會蓋走我填咗成朝的嘢**」。
> 一個答不到這條問題的名稱，等於逼使用者每次都要問人或者索性不敢撳。

---

**症狀**

系統同時有「**重新讀取職事表**」（填寫介面的按鈕）與「**從職事表補抓**」
（試算表選單）。兩個名都是「職事表 ＋ 一個動詞」，使用者完全分不出：
...
逐項查清楚之後，兩支**確實不同**（範圍不同、地方不同、有沒有衝突偵測不同），
但有一點很值得記住：

> **兩個都不會覆寫人手填的值。**

「重新讀取職事表」只重畫畫面上的唯讀區塊（唯一的寫入是
`PROGRAM_TEMPLATE_ID` 原本空白時把推斷結果寫回去）；「從職事表補抓」
**只填空白格**。即是說，使用者最怕那件事，兩個都不會發生——但**兩個名都
沒有講**。

**根本原因**

名稱寫的是**實作在做什麼**（「讀取」「補抓」），不是**使用者要決定什麼**
（「安唔安全？」「範圍幾大？」）。這是介面命名最常見的一種錯位：
寫名的人腦裏想住程式碼，撳的人腦裏想住風險。

**修法**

| 舊名 | 新名 |
|---|---|
| 重新讀取職事表 | **重讀職事表（本主日，不覆寫）** |
| 從職事表補抓（選單） | **補抓空白的事奉欄位（整季，不覆寫已填的）** |
| 從職事表補抓（按鈕） | **補抓空白欄位（不覆寫）** |

每一個名都答齊兩件事：**範圍**（本主日／整季）與**安全性**（不覆寫）。
另外兩顆按鈕的 `title` 與選單那個確認框的**第一句**都再講一次。

⚠️ 順帶一提，同一輪加的「立即整理舊季度」亦跟同一條規則改成
「**立即整理舊季度（封存，不刪資料）**」——「封存」兩個字在別的系統經常
等於「刪走但留備份」，幹事沒有理由知道我們這裏不是。

---

## 事故四十六：斷言依賴真實時間流逝，而測試跑得比它快

**結論先寫在前面：**

> **一條靠「等夠時間」才成立的斷言，等於把測試的對錯交給執行速度。**
>
> 機器快一點、慢一點，同一段程式碼就一時綠一時紅。而更麻煩的是：
> 它紅起來的樣子**不像時間問題**——它會扮成另一個功能壞了。
>
> 修法永遠是同一句：**不要等，直接把判斷基準搬到你要的位置。**

---

### 一、要記住的 bug class

「防重複」「冷卻時間」「快取有效期」「N 秒內只准一次」這一類機制，全部
都是拿「現在」減「上一次」來比。要測「已經超出視窗」那一邊，有兩條路：

| | 做法 | 問題 |
|---|---|---|
| ❌ | 真的等夠 N 秒 | 慢（自測機有時間預算）、脆（機器快就等不夠）、而且**紅起來不像時間問題** |
| ✅ | 把「上一次」那個時間戳**往前撥 N+1 秒** | 快、穩、而且撥了多少寫得出來，變成證據 |

本專案的做法是 `selfTestRewindPublishStamp_()`：只撥沙盒那一個主日的
`PUBLISH_LAST|<主日>`，撥不到就回 `ok:false` 由呼叫方報「驗證不到」。
**三重保護**（只准沙盒主日、只改那一個鍵、撥不到不當成通過）是刻意的——
一支可以改狀態的測試輔助函式，就是一個後門。

⚠️ 撥的幅度要**由 Config 的現值算出來**（`(dedupSec + 60) * 1000`），
不可以寫死一個數字。寫死 `10000` 的話，有人把 `PUBLISH_DEDUP_SEC` 改成
`60`，那條測試就靜靜地變成測另一件事。

---

### 二、這一次的實際成因**不是**時間（值得記下來的部分）
...
第一眼的判斷是「等不夠 30 秒，走了防重複那條路」——**那個判斷是錯的**，
而且錯得很合理，因為症狀「發了兩次、版本沒有加」正是防重複的樣子。

拆穿它的是實際那一句的**前半截**：「**沒有被擋**」。走了防重複那條路的
話，回傳的是 `duplicate: true`，`describePublishBlock_()` 會報
「竟然被擋住（防重複）」。既然它說沒有被擋，那就不是防重複。

真正的成因在上一輪：R-037 §2.2 要求「正式報表排除 `IS_SELFTEST=TRUE`」，
於是 `readOfficialPublishLogRows_()` 被接到四處。其中一處是
`executePublish_()` **數版本號**那一句。結果：

- 自測發佈寫的行全部 `IS_SELFTEST=TRUE`；
- 數版本號時把它們全部濾走 → 每次都數到 0 行 → **每次都算第 1 版**；
- `PublishLog` 出現同一個主日兩行 `VERSION_NO=1`。

**根本原因是一個分類問題：「這是不是正式紀錄」與「這一行的版本號會不會
跟我撞」是兩條不同的問題，答案不同。** 版本號是**寫入**路徑，不是報表；
它要問的是「同一堆行入面最大是幾多」。把報表用的濾網搬去寫入路徑，
就等於用錯了問題去拿答案。

修法：`publishLogRowsOfKind_(rows, isSelfTest)`——正式的跟正式的比，
自測的跟自測的比。R-037 §2.2 的原意（正式版本號不受自測影響）完整保住，
而自測的版本號也會正常遞增。

⚠️ 順帶把 `isSelfTestRun` 收成**一個變數**，數版本號同貼 `IS_SELFTEST`
標籤都用它。本來兩處各自呼叫一次 `isSelfTestMasterFileId_()`——兩次判斷
中間 Config 一改就會對不上，寫出一行標住「正式」但版本號在自測堆裏數出來
的紀錄，事後查不出。
...
1. **診斷要拿實際輸出的每一個字，不是只拿它的形狀。** 這一次
   「沒有被擋」四個字就是全部答案，但形狀（發兩次沒加版本）指向另一個地方。
   斷言的訊息寫得夠細，值就在這裏——**它救得回一次錯誤的診斷**。
2. **一輪修改接上四個呼叫點的時候，要逐個問「這一處問的是同一條問題嗎」。**
   四處之中三處問的是「這是不是正式紀錄」（報表、狀態列、自我檢測），
   只有一處問的是別的。同一支函式接上去，三對一錯。
   見事故三十一（同一個洞散在八個呼叫點）。

---

## 事故四十七：把一個欄位由「介面填」改成「別處為來源」，等於安排了一次清空

**結論先寫在前面：**

> **改變一個欄位的「來源」，就是在安排一次資料遷移。**
>
> 新來源一開始係空嘅。如果規則係「跟隨新來源最新值」，而你冇先把舊資料
> 搬過去——**第一次同步就會把嗰欄清空**，而且每一步都完全按規格運作，
> 一個錯誤訊息都唔會有。
...
R-043／R-045 把九個欄位（讀經經文、證道講題、回應詩歌、詩班曲名、浸禮六欄）
由「介面填」改成「內容表為來源、介面可覆寫」。

第三種模式的規則 2 係：**沒有覆寫的欄位，每次匯入自動跟隨內容表最新值。**

而新分頁一建立就係空嘅。所以：

```
新分頁（空）→ 匯入 →「跟隨最新值」→ 九欄全部變成空白
```

⚠️ 而既有季度（`2027T4`、`2028T4`、`2030T1`、`2030T2`）嗰九欄**已經填咗嘢**。

**點解現有嘅保護擋唔到**

系統本來有一道「分頁層空白保護」：**整張分頁全空就整張跳過**。
呢道保護喺呢個情境下**完全無效**——一旦有人填咗其中一個主日，
分頁就唔算「全空」，其餘主日就即刻失去保護。

保護嘅粒度係「分頁」，而風險嘅粒度係「主日 × 欄位」。**粒度對唔上嘅保護，
喺最危險嗰一刻啱啱好唔生效。**

**修法**

`建立／刷新本季內容表` 喺**首次建立新分頁**時，一律把 `BulletinWeeks` 該季
現有嘅值逐行回填落新分頁，而且：

- 只喺**新建**嗰陣做（已經存在嘅分頁一格都唔覆蓋——覆蓋就等於用週報舊值
  蓋走同工啱啱填嘅嘢）；
- 寫 `AuditLog`，講明回填咗幾多行；
- **喺確認對話框講出嚟**：「將會新建分頁『崇拜程序』，並由週報回填 8 行
  現有資料」。幹事撳之前唔知道系統會幫佢填嘢入內容表，見到之後會以為
  同工已經填過。

⚠️ 預覽同實際用**同一條數法**，而且有測試逐個比對兩個數字。預覽講 8 行、
實際回填 7 行，冇人查得出。
...
1. **「換來源」嘅 checklist：** 舊資料喺邊？新來源一開始係咩狀態？
   第一次同步會發生咩事？——三條問題答唔齊，就唔好開始寫。
2. **保護嘅粒度要同風險嘅粒度一樣。** 「整張全空就跳過」擋唔到
   「其中一行有嘢、其餘被清空」。寫保護嗰陣要問：**我擋嘅係邊一層？
   真正會出事嗰層係唔係同一層？**
...
90. **把「決定處理哪一季／哪一個對象」的退回鏈，拿去決定「向外部系統要
    哪一季／哪一個對象的資料」。** 前者退回是方便（最多顯示錯一季，一眼
    看得出）；後者退回是**拿錯資料**（別一季的真實人名印在週報上，而系統
    報一切正常）。兩者一定要分開，而且要有靜態檢查守住
    （`tools/lint-roster-quarter.js`）。見事故四十一。
91. **一份清單被兩條不同的問題共用。** `rosterDerivedWeekFieldKeys_()` 同時
    被當成「補抓會填哪幾欄」與「有值就代表讀過職事表」，而那三欄之中有兩欄
    不讀職事表都一定有值——於是後一條問題必然答錯。**問題不同，清單就要
    不同**，並且各自寫明自己答的是哪一條。
92. **把「從來沒有算過」正規化成一個正常值。** `String(x || '') || 'OK'`
    這種寫法會令一批從未查證過的資料，在報告上與查證過沒事的資料一模一樣。
    「未算過」要有自己的值，而且一定要印出來。
93. **一句話同時代表「做了 0 件」與「本來就已經齊了」。** 「已建立 0 個
    主日」在工作表有 13 行的時候是誤導。新增數與略過數要一齊講。
94. **上游未齊時，靜靜地回一個「全部 0」的結果。** 匯入回「新增 0、修改 0、
    刪除 0、不變 0」看起來像「已經一致」，其實是「範圍是空的，什麼都沒有
    比較過」。**空範圍要當成錯誤講出來，不可以當成一致。**
95. **同一條問題有幾個地方各自去問。** 「這一季有哪幾個主日」曾經有九個
    呼叫點各自呼叫同一支只讀職事表的函式；答案的定義一改，就要改九次，
    而九次之中改漏八次。**一條問題一個入口**，並用靜態檢查守住。見事故四十二。
96. **「取不到」與「真的沒有」共用同一個回傳值。** 兩者都是空陣列的話，
    呼叫方不可能分得出，於是一定會把前者當成後者。要有一個明確的
    `NONE`／`ok:false`，而且呼叫方一定要處理它。
97. **算好了證據卻沒有印出來。** `I03` 一直知道是哪一項、兩個數字各是幾多，
    但把不變量失敗寫進紀錄那一段只抄 `expected`／`actual`。**證據在最後一步
    被扔掉**，比一開始就沒有算更冤枉——加一行就修好，卻報了 25 次都沒人查得到。
98. **兩路對數的「獨立性」會隨功能改動而失效。** `N01` 本來是「一路走職事表、
    一路走本試算表」，R-036 之後職事表未有該季時兩路會落到同一個來源，
    自己對自己一定對得上。**寫兩路對數的時候要同時寫「什麼情況下這兩路不再
    獨立」**，並在那種情況報「不適用」，不是報綠。
99. **用間接特徵去圈一批「本身就有身分標記」的資料。** 沙盒的 `PublishLog`
    行帶住 `IS_SELFTEST=TRUE`，清理時卻用「日期落在沙盒季度之內」去圈；
    沙盒季度一改，舊行就再也圈不中。與事故三十九同一類。
100. **暫時改一個全域設定去做測試，跑完再還原。** `try/finally` 擋不住
     **執行被硬中斷**（Apps Script 六分鐘上限、使用者撳停、配額用盡）——
     那一刻你的程式碼沒有機會執行任何東西，借了的東西就永遠不還。正確
     做法是根本不借：把它變成一個**只活在這一次執行之內**的參數／記憶體
     變數，執行一死它就跟住死。見事故四十三。
101. **最敏感的那一張表反而沒有稽核。** `Config` 一格被改就足以令正式輸出
     指向錯的地方，而外表完全看不出分別。凡是會影響外部可見結果的設定，
     改動必須逐格記錄——**包括在試算表介面人手改的那些**（那些不經
     `setConfig()`，要靠 `onEdit` 才記得到）。
102. **先寫值、後寫稽核。** 兩者之間被中斷，就會「改了而沒有紀錄」。
     反過來（先稽核後寫值）最壞情況是多一筆「打算改但沒有改成」的紀錄，
     那是看得見、查得到的，比無聲無息好得多。
103. **會自動改寫使用者設定的機制，沒有一份短到一眼數得完的白名單。**
     初始化「自動更新系統種下的預設值」這種機制，範圍每闊一格，就多一道
     「系統會自己改你設定」的門。白名單要短，加鍵時要連測試一齊改，逼人停一停。
104. **系統本來就有足夠資料自己發現問題，卻從來沒有人拿去比對。**
     `PublishLog` 一直記住每一次真實發佈用的是哪一個檔案，只要拿它反查
     `Config` 就即刻看得出那一格被換走了。**查不到的東西要加工具；
     查得到卻沒有查的，要加規則。**
105. **權限系統把唯一有能力修好它的人擋在門外。** 「管理者」與「名單上的人」
     是兩個集合，前者一定要包含在後者——而且要靠程式**無論如何都放行**，
     不是靠人記得去加。⚠️ 加那條放行時要**排在名單判斷之前**：排在之後的話，
     名單非空那條路仍然會先 return，等於沒有加。見事故四十四。
106. **「你沒有權限」而不講「你是誰」。** 這一類問題最常見的原因根本不是權限，
     是瀏覽器登入了另一個帳戶。多印一行「你目前登入的是：xxx」，多數情況
     一眼就看得出；不印的話，看的人完全沒有辦法自助排解。
107. **把一個算不準的估算講到好像算得準。** 頁數要靠排版引擎，Apps Script
     算不到。輸出一定要標明「估算」，而且**不可以阻止**使用者繼續。
     裝成算得準，比不報更差——報錯一次之後就沒有人再信它。
108. **兩個互斥的分支，其實應該是聯集。** `if (A) return X; return Y;`
     寫成這樣，Y 就只在 A 不成立時走得到。要判斷「是不是應該兩者都試」，
     問一句：**兩個條件同時成立時，正確答案是什麼？**
109. **掃描產出物時，把格式本身造成的重複當成內容重複。** OOXML 的
     `mc:AlternateContent` 會令同一段出現兩次（Choice 與 Fallback），
     不先挖走就會滿屏假警報，真正的問題反而被淹沒。⚠️ 挖的時候要數深度，
     不可以用非貪婪正規式——那些結構會巢狀。
110. **功能名稱寫「實作在做什麼」，不是「使用者要決定什麼」。** 幹事最怕
     的不是「這一支做什麼」，是「**它會不會蓋走我填咗成朝的嘢**」。名稱
     答不到那條問題，使用者就每次都要問人，或者索性不敢撳。見事故四十五。
111. **「封存」「歸檔」「清理」這類字眼，沒有講清楚是不是等於刪除。**
     在別的系統它們經常等於「刪走但留備份」。如果你的實作是「只是不顯示」，
     一定要在**名稱本身**講（「封存，不刪資料」），不是只寫在說明文件。
112. **把一個「預設不顯示」的機制做到沒有出路。** 全部季度都封存之後，
     下拉變成空白——使用者分不出「資料不見了」與「資料被封存了」。
     空清單一定要有一句**講得出下一步**的提示，不是一個空的下拉。
113. **自動流程替使用者做判斷。** 「那一季可以收起了嗎」是判斷，不是規則。
     自動跑的時候應該**略過並提醒**，讓人手確認；只有人手觸發那一條路才
     可以 force。
114. **「應該受保護」的清單，把人手要填的表也算進去。** 保護 `Recipients`、
     `PersonDisplay` 這類幹事要維護的表，不是加強保護，是**弄壞流程**。
     加保護之前先問：這張表究竟係機器寫，定係人手填？
115. **斷言依賴真實時間流逝，而測試跑得比它快。** 「N 秒內只准一次」這一類
     機制，要測「已經超出視窗」那一邊，**不要等**——把上一次的時間戳往前撥
     N+1 秒。撥的幅度要由 Config 現值算出來，寫死一個數字的話，Config 一改
     那條測試就靜靜地變成測另一件事。見事故四十六。
116. **把「報表用的濾網」搬去「寫入路徑」。** 「這是不是正式紀錄」（報表問的）
     跟「這一行會不會跟我撞」（編號問的）是兩條不同的問題，答案不同。同一支
     濾網接上去，看起來全部合理，但其中一處會靜靜地答錯。一輪修改接上多個
     呼叫點的時候，逐個問一次「這一處問的是同一條問題嗎」。見事故四十六。
117. **同一個判斷在一支函式裏做兩次。** 兩次之間任何東西一改就會對不上，
     而寫出來的那一行自己不會講——收成一個變數，兩處共用。見事故四十六。
118. **同一支函式同時服務「有 UI」與「沒有 UI」兩個環境。** Web App 沒有
     `SpreadsheetApp.getUi()`，一叫就爆——而爆的地方通常離改動很遠。
     共用的核心一律**只回傳結果物件**，對話框留在選單那一層。用靜態掃描
     釘住比靠人記得住可靠。見 R-038。
119. **把「只保留最新一份」的報告表，接給一個很多人會撳的按鈕。**
     `Diagnostics` 每次執行清空重寫。幹事在填寫介面撳一下唯讀報告，就把
     IT 剛跑完的診斷清走，而且沒有任何提示——兩邊都以為自己看的是自己
     那一份。給人看的即時報告應該回傳資料，不是寫進一張共用的表。
120. **預覽與實際各算一次。** 預覽講「新增 13 行」而實際新增 12 行，
     沒有人查得出差在哪裏。預覽一定要跟執行**走同一組底層函式**，
     而且要有一條測試把兩個數字直接比對。
121. **把一個欄位由「介面填」改成「別處為來源」，而冇先搬舊資料過去。**
     新來源一開始係空嘅，而規則多數係「跟隨來源最新值」——第一次同步就
     清空，而且每一步都按規格運作，一個錯誤訊息都冇。換來源之前答齊三條：
     舊資料喺邊？新來源初始狀態係咩？第一次同步會點？見事故四十七。
122. **保護嘅粒度同風險嘅粒度對唔上。** 「整張分頁全空就跳過」擋唔到
     「其中一行有嘢、其餘被清空」——一旦有人填咗一行，保護就即刻失效，
     啱啱好喺最危險嗰一刻。寫保護嗰陣問：我擋嘅係邊一層？見事故四十七。
123. **同一個欄位有兩個編輯介面，而兩個寫唔同地方。** 填寫介面同季度填寫表
     都改得到嗰十欄——一個寫 `FieldOverride`、一個寫 `BulletinWeeks` 嘅話，
     就係同一格有兩個真相來源。加新嘅寫入路徑之前，先數清楚**而家有幾多個
     地方寫得到呢一格**。
124. **拿「現值」做比較基準，而現值本身已經係比較嘅結果。** 覆寫值寫返落
     原本嗰格之後，再拿嗰格去同「來源」比就永遠相等，覆寫記錄從此唔會再
     更新。要另外記低「覆寫嗰陣來源係咩」（`SOURCE_VALUE_AT_OVERRIDE`／
     `ROSTER_VALUE_AT_OVERRIDE`）。
