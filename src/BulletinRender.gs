...
function generateBulletinDocx_(isoDate) {
  ...
  // ---- R-032：內容份量估算與重複段落偵測 ----
  // ⚠️ 兩者都**只是提示，一定不會阻止產生**。重複段落那一項的價值高於
  //    頁數提示：複製漏刪是一種**印出來才見到**的錯——成品多了兩頁，
  //    而每一步都沒有報過錯。R-047 之後連編號前綴不同的重複都捉得到
  //    （範本硬寫那句沒有編號，內容表印出來那句帶「1.」）。
  //
  //    ⚠️ 這裏本來寫住「88 期真實週報之中有三期是複製漏刪多了兩頁」——
  //    **那個結論已經更正（D-6，2026-09-08）**：對照真印刷版 PDF 逐頁看過，
  //    那些「重複」是 Word 文字方塊同時存 mc:Choice 與 mc:Fallback 造成的
  //    純文字擷取假象，印出來只有一次。成因是當年那個一次性分析腳本沒有
  //    挖走 AlternateContent；出貨的偵測器（OutputAssert.gs）一直都有挖。
  //    見 docs/需求登記_第二批.md R-032 與 docs/測試1發現與待辦.md 第九節。
